...
class HorizontalScrollerView : UIView  {
    ...
    func reload() {
        ...
        contentViews = (0..<dataSource.numberOfViews(in: self)).map {
            ...
            view.frame = CGRect(x: CGFloat(xValue), y: ViewConstants.Padding, width: ViewConstants.Width, height: ViewConstants.Height)
            scroller.addSubview(view)
            xValue += ViewConstants.Width + ViewConstants.Padding
            return view
        }
